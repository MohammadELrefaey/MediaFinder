...
class SQLiteManager{
   ...
   func insertLastSearchResult(mediaArr: [Media]) {
    ...
    //MARK:- media type
    func creatMediaTypeTable() {
        let createTable = self.mediaTypeTable.create { (table) in
            table.column(self.id, primaryKey: true)
            table.column(self.mediaType)
            table.column(self.scStatus)
        }
        do {
            try self.DataBase.run(createTable)
            print("Media Type table created")
        } catch {
            print(error)
        }
    }
    
    func insertMediaType(mediaType: String, scStatus: Int) {
            let insertMediaType = self.mediaTypeTable.insert(self.mediaType <- mediaType, self.scStatus <- scStatus )
            do {
            try self.DataBase.run(insertMediaType)
            print("mediaType inserted")
        } catch {
            print(error)
            }
        }
    ...
    func getMediaType() -> String? {
        do {
            let mediaTypes = try self.DataBase.prepare(self.mediaTypeTable)
            for mediaType in mediaTypes {
                let savedMediaType = mediaType[self.mediaType]
    ...
    func getScStatus() -> Int? {
        do {
            let scStatuses = try self.DataBase.prepare(self.mediaTypeTable)
            for scStatus in scStatuses {
                let savedscStatus = scStatus[self.scStatus]
                    return savedscStatus
                }
        } catch {
            print(error)
        }
